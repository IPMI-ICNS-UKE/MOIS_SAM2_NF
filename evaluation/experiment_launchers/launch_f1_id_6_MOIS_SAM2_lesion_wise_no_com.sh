...
for TEST_SET_ID in "${TEST_SET_IDS[@]}"; do
    ...
    python ../pipelines/evaluation_pipeline.py --network_type $NETWORK --no_prop_beyond_lesions $NO_PROP_BEYOND_LESIONS --exemplar_use_com $USE_COM_DECODER --min_lesion_area_threshold $MIN_LESION_AREA --filter_prev_prediction_components $FILTER_EXEMPLAR_PREDICTION --exemplar_use_only_prompted $USE_ONLY_PROMPTED_EXEMPLARS --exemplar_inference_all_slices $INFERENCE_ALL_SLICES --exemplar_num $NUMBER_OF_EXEMPLARS --fold $FOLD --evaluation_mode $EVAL_MODE --num_lesions $NUM_LESIONS --num_interactions_per_lesion $NUM_INTERACTIONS_PER_LESION --test_set_id $TEST_SET_ID --save_predictions --use_gpu
done
